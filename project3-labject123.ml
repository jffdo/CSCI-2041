...
let makeParser path = 
  ...
  in let token = ref (nextToken ())
in 

let rec nextThing () =
  match ! token with
  | CloseParenToken -> raise (CantParse "CloseParenToken")
  | EndToken -> raise (CantParse "Unexpected end of file")
  | NumberToken(n) ->  token := nextToken(); Number n
  | OpenParenToken -> token := nextToken(); nextThings()
  | SymbolToken("nil") ->  token := nextToken(); Nil
  | SymbolToken(s) ->  token := nextToken(); Symbol s
and
nextThings () = 
  match ! token with 
  | CloseParenToken -> token := nextToken(); Nil
  | EndToken -> raise (CantParse "Unexpected end of file")
  | _ -> let h = nextThing() in Cons(h, nextThings());
in nextThing;;
...
let rec printingThings things =
  ...
  | Nil -> printf ")"
  | _ -> raise BadThing
and printThing thing = 
  printingThing thing; printf "\n"
and
  printingThing thing =
  match thing with
  | Closure(p,b,e) -> printf "[Closure]"
  | Cons(a, b) -> printf "("; printingThings thing
  | Nil -> printf "nil"
  | Number(n) -> printf "%i" n
...
(* COMMAND ARGUMENTS. Call the continuation ETC on every STRING argument from
  ...
  let repl () = commandArguments (fun p -> let nextThing = Parser.makeParser p in 
  try 
    let rec read thing =
      match thing with 
      | Symbol "end" -> ()
      | _ -> let value = Evaluator.evaluate(thing) in Printer.printThing (value); read (nextThing())
    in read (nextThing ())
  with
    | Evaluator.EvaluatorError s -> printf "%s: evaluator error %s\n" p s
    | Parser.CantParse s -> printf "%s: parser error %s\n" p s
    | Printer.BadThing -> printf "%s: printer error\n" p
    | _ -> printf "%s: internal error\n" p
    );;

  
  (* Printer.printThing (Evaluator.evaluate (nextThing ())));; *)
